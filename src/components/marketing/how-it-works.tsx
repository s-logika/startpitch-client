...
const STEPS = [
  {
    ...
    color: "text-brand-blue/25 group-hover:text-brand-blue",
  },
  {
    step: "02",
    ...
    color: "text-brand-green/25 group-hover:text-brand-green",
  },
  {
    step: "03",
    title: "Match & connect",
    ...
    color: "text-brand-sky/25 group-hover:text-brand-sky",
  },
  {
    step: "04",
    ...
    color: "text-brand-orange/25 group-hover:text-brand-orange",
  },
] as const;
  ...
  return (
    <section id="how-it-works" className="scroll-mt-20 border-t bg-muted/30">
      <div className="mx-auto max-w-6xl px-4 py-24 sm:px-6">
        ...
        >
          {STEPS.map((item) => (
            <motion.div key={item.step} variants={fadeUp} className="group relative">
              <span
                className={`inline-block text-3xl font-bold transition-all duration-300 group-hover:scale-110 ${item.color}`}
              >
                {item.step}
              </span>
